...
namespace aula_01.Service
{
    public class BlogServiceImpl : IBlogService
    {
        ...
        private List<BlogPost> order( IEnumerable<BlogPost> posts){
            return posts.OrderByDescending(e => e.PostId).Take(3).ToList();
        }

        public List<BlogPost> GetOlderPosts(int olderPost)
        {
            var teste = Posts.Where(post => post.PostId < olderPost );
           return order(teste);
        }
        ...
        public string GetPostText(string link)
        {
            ...
            return File.ReadAllText($"{_env.ContentRootPath}/wwwroot/posts/Posts/{post.PostId}_post.md");
        }
    }
}